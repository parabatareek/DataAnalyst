# Решите задачу составления набора жанров более компактным методом — в выпадающем списке.
# Добавьте в дашборд элемент dcc.Dropdown для компоновки набора жанров игр.
# Задайте multi = True и id = 'genre_selector'.
# Для dcc.Dropdown задайте параметры options (доступные варианты выбора) и value (выбранные варианты).
# В оба параметра запишите набор уникальных жанров датасета games_grouped.
# Чтобы сформировать options, примените методы list comprehension и unique().
# Сформируйте value методом unique().tolist(). Как выбранные варианты укажите все жанры.
#
# Подсказка:
# Задайте параметры dcc.Dropdown так:
...
app.layout = html.Div(children=[
    ...
    # выбор жанра
    html.Label('Жанры:'),
    dcc.Dropdown(  # напишите код
        options=[{'label': x, 'value': x} for x in games_grouped['genre'].unique()],  # ваш код
        value=games_grouped['genre'].unique().tolist(),  # ваш код
        multi=True,  # ваш код
        id='genre_selector'  # ваш код
    ),
