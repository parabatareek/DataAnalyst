# Редкий дашборд обходится без выбора даты начала и даты окончания периода, для которого отображаются данные.
# С них и начнём.
# Добавьте на дашборд элемент dcc.DatePickerRange с выбором дат начала и окончания периода отображения данных.
# Для элемента установите id = 'dt_selector'.
# Элементу dcc.DatePickerRange установите параметр start_date равным минимальной дате (столбец 'year_of_release') из датафрейма games_grouped.
# Установите параметр end_date равным 2016-01-01.
# Отредактируйте декоратор @app.callback так, чтобы в функцию update_figures передавались параметры 'start_date' и 'end_date' элемента 'dt_selector'.
#
# Подсказка:
# Задайте даты так:
...
app.layout = html.Div(children=[
    ...
    # допишите код ниже
    dcc.DatePickerRange(
        start_date=games_grouped['year_of_release'].dt.date.min(),  # ваш код
        end_date=datetime(2016, 1, 1).strftime('%Y-%m-%d'),  # ваш код
        display_format='YYYY-MM-DD',
        id='dt_selector',  # ваш код
    ),
