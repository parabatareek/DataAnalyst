# Часто заказчику хочется видеть не только абсолютные (например, количество игр, выпущенных по годам),
# но и относительные значения (например, % от общего выпуска игр в год).
# Хорошо сделать так, чтобы абсолютные и относительные значения можно было посмотреть на одном графике.
# Для этого на дашборд нужно добавить элемент управления, позволяющий выбрать режим отображения на графике. Например, radio button.
# Добавьте в дашборд элемент dcc.RadioItems для выбора режима отображения данных.
# Задайте id = 'mode_selector'.
# Отредактируйте декоратор @app.callback так,
# чтобы в функцию update_figures передавался параметр 'value' элемента 'mode_selector'.
# В функции update_figures задайте условие, которое проверяет переменную mode,
# и если она равна 'relative_values',
# то в filtered_data записываются относительные значения (% от общего годового выпуска игр).
#
# Подсказка:
# В функции update_figures задайте условие, которое проверяет переменную mode, и если она равна 'relative_values',
# то в filtered_data записываются относительные значения (% от общего годового выпуска игр).
# Установите:
...
@app.callback(
    ...
    [Input('dt_selector', 'start_date'),
     ...
     Input('mode_selector', 'value'),  # напишите код
     ])
def update_figures(start_date, end_date, mode):
    ...
    if mode == 'relative_values':  # напишите код
