# Настройте элементы фильтрации категориальных переменных.
# Добавьте в дашборд элемент dcc.Checklist для компоновки набора жанров игр.
# Задайте id = 'genre_selector'.
# Отредактируйте декоратор @app.callback так, чтобы в функцию update_figures передавался параметр 'value' элемента 'genre_selector'.
#
# Подсказка:
# Ещё раз проверьте, всё ли правильно в декораторе @app.callback.
# Задайте в декораторе:
# В блоке Output элемент sales_by_year с параметром figure
# В блоке Input элемент dt_selector с параметрами start_date и end_date, элементы mode_selector и genre_selector с параметром value.
...
@app.callback(
    ...
    [Input('dt_selector', 'start_date'),
     ...
     Input('genre_selector', 'value'),  # напишите код
     ])
