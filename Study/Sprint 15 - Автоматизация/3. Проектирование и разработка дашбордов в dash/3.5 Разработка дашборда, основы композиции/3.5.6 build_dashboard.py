# Задайте нужные типы графиков:
# stacked area — go.Scatter, mode = 'lines' и stackgroup = 'one';
# столбчатая диаграмма — go.Bar;
# круговая диаграмма — go.Pie;
# точечная диаграмма — go.Scatter и mode = 'markers'.
#
# Подсказка:
# Ещё раз проверьте типы графиков. Пользуйтесь комментариями в коде:
# Для data_by_genre задайте тип графика go.Scatter с mode = 'lines' и stackgroup = 'one';
# Для sales_by_genre задайте тип графика go.Bar;
# Для data_by_platform задайте тип графика go.Pie;
# Для scores_by_genre задайте тип графика go.Scatter с mode = 'markers'.
...
def update_figures(start_date, end_date, mode, selected_genres, selected_platforms):
    ...
    if mode == 'relative_values':
        ...
        for genre in games_by_genre['genre'].unique():
            data_by_genre += [go.Scatter(x=games_by_genre.query('genre == @genre')['year_of_release'],  # напишите код
                                         y=games_by_genre.query('genre == @genre')['games'],
                                         mode='lines',  # ваш код
                                         stackgroup='one',  # ваш код
                                         name=genre)]
            sales_by_genre += [go.Bar(x=games_by_genre.query('genre == @genre')['year_of_release'],  # ваш код
                                      y=games_by_genre.query('genre == @genre')['total_copies_sold'],
                                      name=genre)]

            # график выпуска игр по платформам
            data_by_platform = [go.Pie(labels=games_by_platform['platform'],  # напишите код
            ...
            for genre in filtered_score['genre'].unique():
                scores_by_genre += [
                    go.Scatter(x=filtered_score.query('genre == @genre')['avg_user_score'],  # напишите код
                               y=filtered_score.query('genre == @genre')['avg_critic_score'],
                               mode='markers',  # напишите код
                               name=genre)]
