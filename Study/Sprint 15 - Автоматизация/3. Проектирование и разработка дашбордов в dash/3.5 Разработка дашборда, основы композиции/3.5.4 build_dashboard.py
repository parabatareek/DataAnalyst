# Напишите логику фильтрации данных. Определите переменные filtered и filtered_score.
#
# Подсказка:
# Заполните query именами переменных из заголовка функции update_figures. Не забудьте про @.
...
def update_figures(start_date, end_date, mode, selected_genres, selected_platforms):
    ...
    filtered = agg_games_year_genre_platform.query(
        'year_of_release >= @start_date and year_of_release <= @end_date')  # напишите код
    filtered = filtered.query('genre in @selected_genres')  # ваш код
    filtered = filtered.query('platform in @selected_platforms')  # ваш код

    filtered_score = agg_games_year_score.query(
        'year_of_release >= @start_date and year_of_release <= @end_date')  # напишите код
    filtered_score = filtered_score.query('genre in @selected_genres')  # ваш код
    filtered_score = filtered_score.query('platform in @selected_platforms')  # ваш код
