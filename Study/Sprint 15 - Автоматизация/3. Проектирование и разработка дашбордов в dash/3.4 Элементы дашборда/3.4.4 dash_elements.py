...
# Графики launches_by_year и sales_by_platform должны быть расположены в одну линию по горизонтали:
# каждый должен занимать шесть (six) колонок лейаута и иметь высоту в 50% ширины дашборда.
# Для элемента div, содержащего графики, укажите правильный класс для отображения элементов в ряд.
#
# Подсказка:
# Помните, что число колонок в лейауте не должно превышать 12. Ширина задается этим числом так: className = 'six columns'.
# Относительная высота элементов — так: style = {'height': '50vw'}.
# Для отображения элементов в ряд укажите класс: className = 'row'.
