...
# Платформа (platform);
# Жанр (genre);
# Продажи по всем регионам (total);
# Сохраните таблицу в переменной games_raw, отсортируйте её по убыванию столбца 'total'. Выведите первые 10 строк.
# Транспонируйте таблицу games_raw и запишите её значения (параметр values) в параметр cells элемента go.Table.
# Задайте таблице id = 'games_by_genre'.
#
# Подсказка:
# Установите:
...
games_raw = games_raw[['name', 'platform', 'genre', 'total']].sort_values(by='total', ascending=False).head(
    10)  # напишите код
